...
const char *Txdbcl::TextC[T_dbMAX+1][2]={
...
Feld::Feld(std::string name, std::string typ, std::string lenge , std::string prec , std::string comment , 
    bool obind, bool vobauto, bool vnnull, std::string vdefa) : 
  name(name),
  typ(typ),
  lenge(lenge),
  prec(prec),
...
Index::Index(string vname, Feld *vfelder, int vfeldzahl) :
  name(vname),
  feldzahl(vfeldzahl),
  felder(vfelder)
{}

  Tabelle::Tabelle(std::string vname, Feld *vfelder, int vfeldzahl, Index *vindices, int vindexzahl,string vcomment,string engine,string charset,string collate,string rowformat) 
:   name(vname),
  comment(vcomment),
  felder(vfelder),
  feldzahl(vfeldzahl),
...
DB::DB(DBSTyp nDBS, string phost, string puser,string ppasswd, string uedb, unsigned int port, const char *unix_socket, unsigned long client_flag,
    uchar obverb,uchar oblog,int versuchzahl)
{
  init(nDBS,phost.c_str(),puser.c_str(),ppasswd.c_str(),uedb.c_str(),port,unix_socket,client_flag,obverb,oblog,versuchzahl);
}

DB::DB(DBSTyp nDBS, const char* phost, const char* puser,const char* ppasswd, const char* prootpwd, const char* uedb, unsigned int port, const char *unix_socket, unsigned long client_flag,uchar obverb,uchar oblog,int versuchzahl): rootpwd(prootpwd)
{
  init(nDBS,phost,puser,ppasswd,uedb,port,unix_socket,client_flag,obverb,oblog,versuchzahl);
}

DB::DB(DBSTyp nDBS, const char* phost, const char* puser,const char* ppasswd, const char* uedb, unsigned int port, const char *unix_socket, unsigned long client_flag,uchar obverb,uchar oblog,int versuchzahl)
{
  init(nDBS,phost,puser,ppasswd,uedb,port,unix_socket,client_flag,obverb,oblog,versuchzahl);
}

void DB::init(DBSTyp nDBS, const char* phost, const char* puser,const char* ppasswd, const char* uedb, unsigned int port, const char *unix_socket, unsigned long client_flag,uchar obverb,uchar oblog,unsigned versuchzahl)
{
  DBS = nDBS;
  fehnr=0;
  string Frage;
...
void DB::pruefrpw(string wofuer, unsigned versuchzahl)
{
    ...
    if (strchr("jJyYoOsSnN",erg)) {
      ...
      if (rootpw2==rootpwd && !rootpwd.empty()) {
        cmd=string("mysql -uroot -h'")+host+"' -e \"grant all on *.* to 'root'@'"+myloghost+
          "' identified by '"+ersetze(rootpwd,"\"","\\\"")+"' with grant option\"";
...
void DB::lesespalten(Tabelle *tab)
{
  char ***cerg;
  //          RS spalt(this,string("SHOW COLUMNS FROM `")+tab->name+"`");
  delete spalt;
  //  spalt=new RS(this,string("SELECT column_name,character_maximum_length FROM information_schema.columns WHERE table_name = '")+tab->name
  ...
  spalt=new RS(this,string("SELECT column_name p0,"
        ...
        "FROM information_schema.columns WHERE table_name = '")+tab->name+"' AND table_schema = '"+db+"' ORDER BY ordinal_position");
  if (!spalt->obfehl) {
    delete spnamen;
    ...
    int spnr=0;
    //    cout<<violett<<"Schema: "<<schwarz<<db<<endl;
    //    cout<<violett<<"Tabelle: "<<schwarz<<tab->name<<endl;
    while (cerg=spalt->HolZeile(),cerg?*cerg:0) {
      spnamen[spnr]=*(*cerg);
      splenge[spnr]=*(*cerg+1);
      /*
         MYSQL_RES *dbres;
         dbres = mysql_list_fields(conn,tab->name.c_str(),tab->felder[spnr].name.c_str());
         cout<<violett<<"spnamen["<<spnr<<"]: "<<schwarz<<spnamen[spnr]<<endl;
         cout<<violett<<" splenge["<<spnr<<"]: "<<schwarz<<(splenge[spnr]?splenge[spnr]:"NULL")<<endl;
         if (dbres) if (dbres->fields->name) cout<<" dbres->fields->name: "<<dbres->fields->name<<endl;
         if (dbres) if (dbres->fields->length) cout<<" dbres->fields->length: "<<dbres->fields->length<<endl;
       */
      spnr++;
    }
  }
...
int DB::prueftab(Tabelle *tab,bool verbose) 
{
  int gesfehlr=0;
  RS rs(this);
  std::stringstream sql;
  // eine Indexfeldlaenge groesser als die Feldlaenge fuehrt zu Fehler (zumindex bei MariaDB)
  for(int i=0;i<tab->indexzahl;i++){
    for(int j=0;j<tab->indices[i].feldzahl;j++){
      for(int k=0;k<tab->feldzahl;k++){
        if (!tab->felder[k].lenge.empty()) if (!tab->indices[i].felder[j].lenge.empty())
          if (!strcasecmp(tab->felder[k].name.c_str(),tab->indices[i].felder[j].name.c_str())) 
            if (atol(tab->felder[k].lenge.c_str())<atol(tab->indices[i].felder[j].lenge.c_str())){
              tab->felder[k].lenge=tab->indices[i].felder[j].lenge;
            }
      }
    }
  }
    ...
    case MySQL:
      {
        ...
        for(int i=0;i<tab->feldzahl;i++) {
          if (!spalt->obfehl)
            for(unsigned j=0;j<spalt->num_rows;j++) {
              if (!strcasecmp(tab->felder[i].name.c_str(),spnamen[j])) {
                if (splenge[j]) 
                  if (atol(splenge[j]) > atol(tab->felder[i].lenge.c_str())) 
                    tab->felder[i].lenge=splenge[j];
                break;
              }
            }
          fstr.resize(fstr.size()+1);
          istr.resize(istr.size()+1);
          tab->felder[i].comment=ersetze(tab->felder[i].comment,"'","\\'");
          //<<"tab->felder[i].comment: "<<tab->felder[i].comment<<endl;
          fstr[i]= "`" + tab->felder[i].name + "` "+
            tab->felder[i].typ+
            ((tab->felder[i].lenge==""||tab->felder[i].lenge=="0")?"":
             (string("(")+tab->felder[i].lenge+
              ((tab->felder[i].prec=="")?"":
               (string(",")+tab->felder[i].prec))
              +")"))
            +(tab->felder[i].nnull  ?  " NOT NULL ":"")
            +(tab->felder[i].defa.empty()&&!tab->felder[i].nnull?"":string(" DEFAULT '")+tab->felder[i].defa+"'")
            +(tab->felder[i].obauto?" AUTO_INCREMENT":" ")
            +(tab->felder[i].obind && tab->felder[i].obauto?" PRIMARY KEY":" ")
            +((tab->felder[i].comment.empty())?"":
                (string(" COMMENT '")+tab->felder[i].comment+"'"));
          if (tab->felder[i].obind && !tab->felder[i].obauto) {
            istr[i]=string(", ADD INDEX `")+tab->felder[i].name+"`(`"+tab->felder[i].name+"`)";
          }
        } // for(int i=0;i<tab->feldzahl;i++)
        MYSQL_RES *dbres = mysql_list_tables(conn,tab->name.c_str());
        if (!dbres->row_count) {
          tab->comment=sersetze(tab->comment,string("'"),string("\\'"));
          sql<<"CREATE TABLE `"<<tab->name.c_str();
          sql<<"` ("<<fstr[0]<<istr[0]<<") COMMENT='";
          sql<<tab->comment
            <<"' ENGINE="<<(tab->engine==""?def_engine:tab->engine);
          sql<<" DEFAULT CHARSET="
            <<(tab->charset==""?def_charset:tab->charset);
          sql<<((tab->charset!=""&&tab->collate=="")?"":" COLLATE=")
            <<(tab->collate==""?(tab->charset!=""?"":def_collate):tab->collate);
          sql<<" ROW_FORMAT="
            <<(tab->rowformat==""?def_rowformat:tab->rowformat);
        ...
        for(int gspn=0;gspn<tab->feldzahl;gspn++) { // geplante Spalten
          binaer gefunden=falsch;
          for(unsigned j=0;j<spalt->num_rows;j++) { // reale Spalten
            if (!strcasecmp(tab->felder[gspn].name.c_str(),spnamen[j])) {
              gefunden=wahr;
              break;
            }
          }
          if (!gefunden) {
            ...
            sql<<"ALTER TABLE `"<<tab->name<<"` ADD "<<fstr[gspn];
            if (gspn) sql<<" AFTER `"<<tab->felder[gspn-1].name<<"`";
            else sql<<" FIRST";
            sql<<istr[gspn];
        ...
        for(int gspn=0;gspn<tab->feldzahl;gspn++) { // geplante Spalten
          ...
          for(unsigned spnr=0;spnr<spalt->num_rows;spnr++) { // reale Spalten
            if (!strcasecmp(tab->felder[gspn].name.c_str(),spnamen[spnr])) { // Feldnamen identisch
              if ((tab->felder[gspn].lenge!="0" && !tab->felder[gspn].lenge.empty()) &&
                  atol(tab->felder[gspn].lenge.c_str()) > (long)atol(splenge[spnr])) {
                aendere=wahr;
              }
              if (gspn) { // Verschiebung erst ab der zweiten geplanten Spalte reicht auch und vermeidet Speicherunterlaeufe
                verschieb=wahr; 
                if (spnr) if (!strcasecmp(tab->felder[gspn-1].name.c_str(),spnamen[spnr-1])) verschieb=falsch;
                if (verschieb) spnr--;
              }
          ...
          if (verschieb || aendere) {
            sql.str(std::string()); sql.clear();
            sql<<"ALTER TABLE `"<<tab->name<<"` MODIFY "<<fstr[gspn];
            if (gspn) sql<<" AFTER `"<<tab->felder[gspn-1].name<<"`";
            else sql<<" FIRST";
            /*int erg=*/rs.Abfrage(sql.str(),verbose?255:1);
            gesfehlr+=rs.obfehl;
            ...
            if (aendere) {
              if (!istr[gspn].empty()) {
                RS rloesch(this,string("DROP INDEX `")+tab->felder[gspn].name +"` ON `"+tab->name+"`");
                sql.str(std::string()); sql.clear();
                sql<<"ALTER TABLE `"<<tab->name<<"`"<<istr[gspn].substr(1);
        ...
        for(int i=0;i<tab->indexzahl;i++) {
          ...
          RS rind(this,string("SHOW INDEX FROM `") + tab->name+"` WHERE KEY_NAME = '"+ tab->indices[i].name +"'");
          if (!rind.obfehl) {
            uchar obneu=0;
            if (!rind.result->row_count){
              ...
              for(int j=0;j<tab->indices[i].feldzahl;j++) {
                ...
                if (!*erg) {
                  obneu=1;break;
                } else if (strcasecmp(tab->indices[i].felder[j].name.c_str(),*(*erg+4))) { 
                  ...
                  obneu=1;break;
                } else if (tab->indices[i].felder[j].lenge!="" && *(*erg+7)) {
                  if (tab->indices[i].felder[j].lenge!=*(*erg+7)) { 
                    obneu=1;break;
                  }
                }
              }
              ...
              if (obneu) {
                RS rloesch(this,string("DROP INDEX `")+tab->indices[i].name +"` ON `"+tab->name+"`");
              }
            }
            ...
            if (obneu) {
              RS rindins(this);
              sql.str(std::string()); sql.clear();
              sql<<"CREATE INDEX `"<<tab->indices[i].name<<"` ON `"<<tab->name<<"`(";
              for(int j=0;j<tab->indices[i].feldzahl;j++) {
                sql<<"`"<<tab->indices[i].felder[j].name<<"`";
                if (tab->indices[i].felder[j].lenge!="0" && tab->indices[i].felder[j].lenge!="") {
                  sql<<"("<<tab->indices[i].felder[j].lenge<<")";
                }
                if (j<tab->indices[i].feldzahl-1) {
                  sql<<",";
                }
              } // for(int j=0;j<tab->indices[i].feldzahl;j++) 
              sql<<")";
...
uchar DB::tuerweitern(const char* tab, string feld,long wlength,uchar obstumm)
{
  stringstream korr;
  string lenge;
  korr<<"SELECT character_maximum_length, data_type,is_nullable,column_default,column_comment FROM information_schema.columns WHERE table_schema='"<<
    db<<"' AND table_name='"<<tab<<"' AND column_name='"<<feld<<"'";
    ...
    while(cerg= spaltlen.HolZeile(),cerg?*cerg:0) {
      if (*(*cerg+0)) {
        ...
        if (atol(lenge.c_str())<wlength) {
          Log(string("Erweitere Feld: ")+tab+"."+feld+" von: "+lenge.c_str()+" auf: "+ltoan(wlength),1,1);
          korr.str(std::string()); korr.clear();
          if (*(*cerg+1) && *(*cerg+2)) {
            korr<<"ALTER TABLE `"<<tab<<"` MODIFY COLUMN `"<<feld<<"` "<<*(*cerg+1)/*data_type*/<<"("<<wlength<<") "<<
              (!strcasecmp(*(*cerg+2),"yes")?"NULL":"NOT NULL")<<" "<<(*(*cerg+3)?string("DEFAULT '")+*(*cerg+3)+"'":"")<<
              " COMMENT '"<<ersetze(string(*(*cerg+4)),"'","\\'")<<"'";
            RS spaltaend(this,korr.str(),obstumm);
            if (spaltaend.fnr==1074) {
              korr.str(std::string()); korr.clear();
              korr<<"ALTER TABLE `"<<tab<<"` MODIFY COLUMN `"<<feld<<"` "<<"MEDIUMTEXT"/*data_type*/<<" "<<
                (!strcasecmp(*(*cerg+2),"yes")?"NULL":"NOT NULL")<<" "<<(*(*cerg+3)?string("DEFAULT '")+*(*cerg+3)+"'":"")<<
                " COMMENT '"<<ersetze(string(*(*cerg+4)),"'","\\'")<<"'";
        ...
        //        cerr<<"Fehler bei der Abfrage der Tabellenlaenge bei Tabelle: "<<tab<<" und Feld: "<<feld<<endl<<" mit: "<<korr.str()<<endl;
        //        return 2; // Fehler bei der Abfrage der Spaltenlaenge
      } // if (**cerg) else
    } // while(cerg= HolZeile(),cerg?*cerg:0) 
  } else {
    cerr<<Txd[T_falsche_Fehlernr]<<spaltlen.fnr<<Txd[T_bei_der_Abfrage_der_Spaltenlaenge_bei_Tabelle]<<tab<<
...
void DB::erweitern(const char* tab, vector<instyp> einf,uchar obstumm,uchar obsammeln, const unsigned long *maxl)
{
  for(uint i=0;i<einf.size();i++){
    ...
    tuerweitern(tab,einf[i].feld,wlength,obstumm);
  }
} // RS::erweitern
...
RS::RS(DB* pdb,const char* psql,uchar obstumm) 
{
  weisezu(pdb);
...
RS::RS(DB* pdb,string psql,uchar obstumm) 
{
  weisezu(pdb);
  Abfrage(psql,obstumm);
}
...
void RS::update(const char* tab, vector< instyp > einf,uchar obstumm,string *bedingung) 
{
  ulong locks=0;

  switch (db->DBS) {
    case MySQL:
      isql=string("UPDATE ")+db->dnb+tab+db->dne+" SET ";
      for(uint i = 0;i<einf.size();i++) {
    ...
    case MySQL:
      {
        ...
        for (int iru=0;iru<2;iru++) { // interne Runde
          ...
          if (!obfehl) {
            ...
            if (fnr==1406){
              db->erweitern(tab,einf,obstumm,0);
...
void RS::insert(const char* tab, vector< instyp > einf,uchar anfangen,uchar sammeln,uchar obstumm,string *id,uchar eindeutig) 
{
  /*
  ...
  // <<"insert in "<<drot<<tab<<schwarz<<" anfangen: "<<(int)anfangen<<" sammeln: "<<(int)sammeln<<endl;
    ...
    switch (db->DBS) {
      case MySQL:
        aut/*id*/<<"SELECT column_name FROM information_schema.columns WHERE table_schema='"<<db->db<<
          "' AND table_name = '"<<tab<<"' AND extra = 'auto_increment'";
        ...
        if (!obfehl) {
          ...
          aut<<"SELECT `"<<autoz<<"` FROM `"<<tab<<"` WHERE ";
          for(uint i=0;i<einf.size();i++){
            if (i) aut<<" AND ";
  ...
  if (anfangen) {
    switch (db->DBS) {
      case MySQL:
        isql=string("INSERT INTO ")+db->dnb+tab+db->dne+'(';
        for(uint i = 0;i<einf.size();i++) {
          if (i) isql+=',';
          isql+=db->dnb+einf[i].feld+db->dne;
        }
    ...
    switch (db->DBS) {
      case MySQL:
        ...
        if (!anfangen) if (isql.empty()) {
          Log(string(drot)+Txd[T_Programmfehler]+schwarz+Txd[T_nicht_anfangen_bei_isql_empty_Aufruf_von_RS_insert_beim_ersten_Mal_ohn_anfangen_bei]+drot+db->dnb+tab+db->dne+schwarz+", einf.size(): "+ltoan(einf.size()),1,1);
          for(uint i=0;i<einf.size();i++){
      ...
      case MySQL:
        {
          ...
          for (int iru=0;iru<2;iru++) { // interne Runde
            ...
            if (!obfehl) {
              ...
              Log(mysql_error(db->conn),(fnr!=1406 && obstumm!=2) || (fnr==1406 && obstumm==255),1);
              if (fnr==1406){
                db->erweitern(tab,einf,obstumm, sammeln || (!sammeln && !anfangen),maxl);
